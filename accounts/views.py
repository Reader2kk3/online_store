    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message = render_to_string('accounts/account_verification_email.html', {
                'user': user,
                'domain': current_site,
                'uid': urlsafe_base64_encode(force_bytes(user.pk)),  # Кодировка первичного ключа
...
# Активация аккаунта через сообщения
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.error(request, 'Активация провалена!')
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            user = Account.objects.get(email__exact=email)          # email__exact - точечная проверка на совпадение

            # Сброс пароля
            current_site = get_current_site(request)
            mail_subject = "Пожалуйста, обновите пароль!"
            ...
            messages.error(request, 'Аккаунт с данной электронной почтой не существует')
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(request, 'Пожалуйста, введите новый пароль')
    ...
    if request.method == 'POST':
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                messages.error(request, 'Пожалуйста, введите правильно данные!')
                return redirect('change_password')
        else:
            messages.error(request, 'Пароли не совпадают!')
            return redirect('change_password')
